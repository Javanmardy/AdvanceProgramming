...
def save_data(data):
    with open("data.json", "w") as file:
        json.dump(data, file, indent=2, cls=CustomJSONEncoder)
...
def object_hook(obj):
    if "__class__" in obj:
        ...
        if class_name == "Restaurant":
            return Restaurant(obj["name"], obj["address"], obj["contact_info"])
        elif class_name == "Menu":
            menu = Menu()
            menu.items = obj["items"]
            return menu
        elif class_name == "MenuItem":
            return MenuItem(obj["name"], obj["description"], obj["price"])
        elif class_name == "Customer":
            return Customer(obj["name"], obj["address"], obj["contact_info"])
        elif class_name == "Order":
            order = Order(obj["customer"], obj["restaurant"], obj["items"])
            order.total_price = obj["total_price"]
            return order
        elif class_name == "OrderHistory":
            order_history = OrderHistory()
            order_history.orders = obj["orders"]
...
if __name__ == "__main__":
    ...
    while True:
        data = main(data)  # Pass the loaded data to the main function
